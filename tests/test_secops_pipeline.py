...
from sigma.pipelines.secops.utils import determine_event_type_event_id, determine_event_type_logsource, get_rule_detection_fields
...
def test_determine_event_type_logsource():
    ...
    rule = SigmaRule.from_dict({"title": "Test", "logsource": logsource, "detection": {"sel": {"field": "value"}, "condition": "sel"}})
...
def test_remove_hash_algo_from_hashes(secops_pipeline):
    ...
    assert rule.detection.detections["selection"].detection_items[0].value[0] == SigmaString("1234567890abcdef1234567890abcdef")
    assert rule.detection.detections["selection"].detection_items[0].value[1] == SigmaString("92429d82a41e930486c6de5ebda9602d55c39986")
    assert rule.detection.detections["selection"].detection_items[0].value[2] == SigmaString("2413fb3709b05939f04cf2e92f7d0897fc2596f9ad0b8a9ea855c7bfebaae892")
...
def test_prepend_metadata_postprocessing(secops_backend_prepend_metadata: SecOpsBackend):
    ...
    assert secops_backend_prepend_metadata.convert_rule(rule)[0] == '(metadata.event_type = "PROCESS_LAUNCH") AND (target.process.file.full_path = "C:\\\\Windows\\\\System32\\\\cmd.exe" nocase)'
